...
export async function setup() {
    ...
    const stopInstalled = settings.hooks["Stop"].some((h) => h.command === stopCommand);
    if (stopInstalled) {
    ...
    for (const matcher of TOOL_USE_MATCHERS) {
        const alreadyInstalled = settings.hooks["PostToolUse"].some((h) => h.command === toolUseCommand && h.matcher === matcher);
        if (!alreadyInstalled) {
...
export async function uninstall() {
    ...
    try {
        ...
        if (settings.hooks?.["Stop"]) {
            settings.hooks["Stop"] = settings.hooks["Stop"].filter((h) => h.command !== stopCommand);
            if (settings.hooks["Stop"].length === 0) {
                delete settings.hooks["Stop"];
            }
        }
        if (settings.hooks?.["PostToolUse"]) {
            settings.hooks["PostToolUse"] = settings.hooks["PostToolUse"].filter((h) => h.command !== toolUseCommand);
...
function getHookScriptPath(filename) {
