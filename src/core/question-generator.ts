...
function buildUserPrompt(context: ContextSummary): string {
  const parts: string[] = ["Here's what the coding assistant just did:"];
  ...
  if (context.commands_run.length > 0) {
    ...
    const cmds = context.commands_run.slice(-5).map((c) =>
...
function callClaude(
  ...
  return new Promise((resolve, reject) => {
    const proc = spawn(
      "claude",
      ["-p", "--model", "haiku", "--system", systemPrompt],
      {
 ...
 */
export async function generateToolUseQuestion(
  toolName: string,
...
export function buildToolUsePrompt(
  ...
  const parts: string[] = ["The coding assistant just performed this action:"];
...
function getToolUseFallbackQuestion(toolName: string): Question {
  switch (toolName) {
    case "Bash":
      return {
        ...
        correctIndex: 1,
        explanation:
          "Shell commands can modify files, install packages, or change system state in ways that may be hard to reverse.",
      };
    case "Edit":
      return {
        ...
        correctIndex: 1,
        explanation:
          "Targeted edits minimize risk by only changing what's needed, leaving the rest of the file intact.",
      };
    default:
      return {
        ...
        correctIndex: 2,
        explanation:
          "You own the code once it's in your project. Understanding it is key for maintenance and debugging.",
      };
  }
}

function getFallbackQuestion(context: ContextSummary): Question {
  if (context.files_touched.length > 0) {
    return {
      question:
        "What's a good practice when modifying multiple files in a single change?",
      choices: [
      ...
      correctIndex: 1,
      explanation:
        "Atomic, related changes are easier to review, test, and revert if something goes wrong.",
    };
  }

  return {
    question:
      "Why is it valuable to understand the code an AI assistant writes for you?",
    choices: [
    ...
    correctIndex: 2,
    explanation:
      "You own the code once it's in your project. Understanding it is essential for maintenance and debugging.",
  };
}
