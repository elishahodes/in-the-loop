...
import type { Frequency, Difficulty } from "../types.js";

const VALID_FREQUENCIES: Frequency[] = ["every", "often", "sometimes", "rarely"];
...
export async function configure(args: string[]): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === "--frequency" && args[i + 1]) {
      const value = args[i + 1] as Frequency;
      if (!VALID_FREQUENCIES.includes(value)) {
        console.error(
          `Invalid frequency: ${value}. Must be one of: ${VALID_FREQUENCIES.join(", ")}`,
        );
        process.exit(1);
      }
      ...
      const value = args[i + 1] as Difficulty;
      if (!VALID_DIFFICULTIES.includes(value)) {
        console.error(
          `Invalid difficulty: ${value}. Must be one of: ${VALID_DIFFICULTIES.join(", ")}`,
        );
        process.exit(1);
      }
