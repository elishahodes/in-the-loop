...
function makeState(stop_count: number, tool_use_count = 0): SessionState {
...
describe("shouldShowQuestion (tool_use trigger)", () => {
  it("does NOT show on first tool use", () => {
    assert.equal(
      shouldShowQuestion("every", makeState(0, 0), false, "tool_use"),
      false,
    );
  });
  ...
  it("skips trivial responses", () => {
    assert.equal(
      shouldShowQuestion("every", makeState(0, 5), true, "tool_use"),
      false,
    );
  });
  ...
  it("shows every 5th with 'every' frequency", () => {
    assert.equal(
      shouldShowQuestion("every", makeState(0, 4), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("every", makeState(0, 9), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("every", makeState(0, 3), false, "tool_use"),
      false,
    );
  });
  ...
  it("shows every 8th with 'often' frequency", () => {
    assert.equal(
      shouldShowQuestion("often", makeState(0, 7), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("often", makeState(0, 15), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("often", makeState(0, 6), false, "tool_use"),
      false,
    );
  });
  ...
  it("shows every 12th with 'sometimes' frequency", () => {
    assert.equal(
      shouldShowQuestion("sometimes", makeState(0, 11), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("sometimes", makeState(0, 23), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("sometimes", makeState(0, 10), false, "tool_use"),
      false,
    );
  });
  ...
  it("shows every 20th with 'rarely' frequency", () => {
    assert.equal(
      shouldShowQuestion("rarely", makeState(0, 19), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("rarely", makeState(0, 39), false, "tool_use"),
      true,
    );
    assert.equal(
      shouldShowQuestion("rarely", makeState(0, 18), false, "tool_use"),
      false,
    );
  });
  ...
  it("uses tool_use_count not stop_count", () => {
    ...
    assert.equal(
      shouldShowQuestion("every", makeState(100, 2), false, "tool_use"),
      false,
    );
